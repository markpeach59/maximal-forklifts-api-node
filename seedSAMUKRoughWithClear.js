...
const forkliftDetailData = [
  ...
  },
  {
    ...
    basePrice: 32900,
    imgName: "Rough2WD.jpg",
    powertrain: "Kubota V3800 Euro 5",
    modeldescription:[{description:"M Series 2WD"}],
    loadcenter:500,
    defaulttyre:"Pneumatic",
    masts: [
  ...
  },
  {
    ...
    engType: "Rough Terrain",
    basePrice: 19800,
    imgName: "AX-SERIES-ROUGH-TERRAIN-18T-25T-35T-2WD.jpg",
   
    modeldescription:[{description:"AX Series 2WD"}],
    loadcenter:500,
    defaulttyre:"Pneumatic",
    ...
    defaultbattery: "153.6V 230A/H Lithium",
    defaultcharger: "Fast",
...
async function seed() {
  ...
  // Clear existing rough terrain entries from Forkliftdetails collection
  console.log("Clearing existing rough terrain entries from Forkliftdetails...");
  ...
  for (let forkliftItem of forkliftDetailData) {
    const forky = new Forkliftdetail(forkliftItem);
    const doc = await forky.save();
    console.log(`Saved forklift detail: ${doc.model}`);
  }

  // Seed forklift range data
  console.log("Seeding forklift range data...");
  for (let rangeItem of forkliftRangeData) {
    const forky = new Forklift(rangeItem);
    const doc = await forky.save();
    console.log(`Saved forklift range: ${doc.range}`);
  }

  mongoose.disconnect();
  console.info("Seed process completed successfully!");
}
