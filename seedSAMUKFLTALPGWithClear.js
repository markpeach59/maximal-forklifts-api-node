...
const forkliftDetailData = [
  ...
  cabin: [
    { cabinoption: "Half Cabin", price: 1020 },
...
async function seed() {
  ...
  console.log("Starting FLTA LPG seed process...");

  // Clear existing LPG entries from Forkliftdetails collection
  console.log("Clearing existing LPG entries from Forkliftdetails...");
  ...
  mongoose.disconnect();
  console.info("FLTA LPG seed process completed successfully!");
}

seed().catch(err => {
  console.error("FLTA LPG seed process failed:", err);
  mongoose.disconnect();
});
