...
async function seed() {
  ...
  console.log("Starting Reach seed process...");

  // Clear existing reach entries from Forkliftdetails collection
  console.log("Clearing existing reach entries from Forkliftdetails...");
  ...
  mongoose.disconnect();
  console.info("Reach seed process completed successfully!");
}

seed().catch(err => {
  console.error("Reach seed process failed:", err);
  mongoose.disconnect();
});
