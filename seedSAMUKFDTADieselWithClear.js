...
// FDTA80 and FDTA100 models (placed at the end as requested)
{
  model: "FDTA80",
capacity: 8000,
engType: "Diesel",
...
async function seed() {
  ...
  console.log("Starting FDTA Diesel seed process...");

  // Clear existing diesel entries from Forkliftdetails collection
  console.log("Clearing existing diesel entries from Forkliftdetails...");
  ...
  mongoose.disconnect();
  console.info("FDTA Diesel seed process completed successfully!");
}

seed().catch(err => {
  console.error("FDTA Diesel seed process failed:", err);
  mongoose.disconnect();
});
